import{F as e,o as t}from"./Stack-Tzgstd5M.js";import{An as n,Cn as r,Qt as i,Tn as a,Xt as o,i as s,wn as c}from"./index-B9A7Z9tD.js";var l=t((0,e().jsx)(`path`,{d:`M19 7H9c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h10c1.1 0 2-.9 2-2V9c0-1.1-.9-2-2-2m0 3c0 .55-.45 1-1 1h-8c-.55 0-1-.45-1-1s.45-1 1-1h8c.55 0 1 .45 1 1m-6 5v-2h2v2zm2 2v2h-2v-2zm-4-2H9v-2h2zm6-2h2v2h-2zm-8 4h2v2H9zm8 2v-2h2v2zM6 17H5c-1.1 0-2-.9-2-2V5c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2v1h-2V5H5v10h1z`}),`TableViewRounded`),u=e=>{let t=s(e);return t?t.name:null},d=e=>Number.isFinite(e)?e:``,f=e=>Number.isFinite(e)?o(e):``,p=e=>e?new Date(e).toLocaleString():``,m=(e,t,n)=>{let r=String(e.id||``).split(`@`)[0].replace(/[^\w.-]/g,``);return`timesheet-${r?`${r}-`:``}${t}-${i(n)}`},h=[{label:`社員ID`,value:e=>e.profile.id},{label:`氏名`,value:e=>e.profile.name},{label:`部署`,value:e=>e.profile.department},{label:`日付`,value:e=>e.dateKey},{label:`曜日`,value:e=>e.weekday},{label:`祝日`,value:e=>e.holidayName},{label:`区分`,value:e=>e.entry.type},{label:`出社`,value:e=>e.entry.clockIn},{label:`退社`,value:e=>e.entry.clockOut},{label:`基本(分)`,value:e=>d(e.calc.basic)},{label:`早出残業(分)`,value:e=>d(e.calc.earlyOvertime)},{label:`早朝深夜(分)`,value:e=>d(e.calc.night)},{label:`合計(分)`,value:e=>d(e.calc.total)},{label:`備考`,value:e=>e.entry.note}],g=e=>e.flatMap(({profile:e,rows:t})=>t.map(t=>({...t,profile:e}))),_=({profile:e,year:t,month:n,rows:i})=>{let o=`${m(e,t,n)}-${a()}.csv`;return c(r(g([{profile:e,rows:i}]),h),o),o},v=({items:e,year:t,month:n})=>{let o=`attendance-book-${t}-${i(n)}-${a()}.csv`;return c(r(g(e),h),o),o},y={"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`},b=e=>String(e??``).replace(/[&<>"']/g,e=>y[e]),x=`
...
<div class="info">
    ...
    ${S(`部署`,e.department)}
    ${S(`役職`,e.position)}
    ${S(`承認状態`,s.approvalStatus?n(s.approvalStatus):`未作成`)}
    ${S(`申請日時`,p(s.submittedAt))}
    ${S(`承認者`,s.approvedBy)}
    ${S(`承認日時`,p(s.approvedAt))}
    ${s.closingStatus===`closed`?`<div><b>締め済</b></div>`:``}
</div>
${s.remarks?`<div class="remarks"><b>承認者備考</b>: ${b(s.remarks)}</div>`:``}
<div class="totals">
    ${C(`出勤`,`${a.workDays}日`)}
    ...
    ${C(`総就業`,o(a.totalWork))}
    ${C(`所定`,o(a.expected))}
    ${C(`差分`,`${a.diff>=0?`+`:``}${o(a.diff)}`)}
    ${C(`早出/残業`,o(a.earlyOvertime))}
    ${C(`早朝/深夜`,o(a.night))}
</div>
<table>
    <thead><tr>
    ...
    <tbody>
        ...
        <tr class="sum">
            ...
            <td class="num">${o(a.basic)}</td>
            <td class="num">${o(a.earlyOvertime)}</td>
            <td class="num">${o(a.night)}</td>
            <td class="num">${o(a.totalWork)}</td>
            <td></td>
        </tr>
    </tbody>
</table>
<div class="foot"><span>${b(e.name)} / ${t}年${r}月</span><span>Recrova</span></div>
</section>`,E=(e,t)=>`<!doctype html>
<html lang="ja"><head><meta charset="utf-8"><title>${b(e)}</title><style>${x}</style></head>
<body>
${t.join(`
`)}
</body></html>`,D=e=>E(`月次タイムシート_${e.year}年${i(e.month)}月_${e.profile.name}`,[T(e)]),O=({items:e,year:t,month:n})=>E(`出勤簿_${t}年${i(n)}月`,e.map(e=>T({...e,year:t,month:n}))),k=e=>{let t=document.createElement(`iframe`);t.setAttribute(`aria-hidden`,`true`),t.title=`印刷プレビュー`,t.style.cssText=`position:fixed;right:0;bottom:0;width:0;height:0;border:0;`,t.srcdoc=e,t.onload=()=>{let e=t.contentWindow;if(!e){t.remove();return}let n=!1,r=()=>{n||(n=!0,window.setTimeout(()=>t.remove(),0))};e.addEventListener(`afterprint`,r,{once:!0}),window.setTimeout(r,6e4),e.focus(),e.print()},document.body.appendChild(t)},A=e=>{k(D(e))},j=e=>{k(O(e))};export{u as a,A as i,j as n,l as o,_ as r,v as t};